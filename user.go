...
// checkIser check whether user is valid
...
func addUser(user User) bool {
	...
	if queryResult.TotalHits() > 0 {
		fmt.Printf("User %s already exists, cannot create dulplicate user. \n", user.Username)
		return false
	}
	...
	_, err = es_client.Index().
		...
		Do(context.Background())
	if err != nil {
		fmt.Printf("ES save user falied %v\n", err)
		return false
	}
...
// if signup success, a new session is created
func signupHandler(w http.ResponseWriter, r *http.Request) {
	fmt.Println("Receive a signup request")
	...
	if u.Username != "" && u.Password != "" && usernamePattern(u.Username) {
		...
		fmt.Printf("Empty password or username or valid username.")
		http.Error(w, "Empty password or username or valid username.", http.StatusInternalServerError)
	}
